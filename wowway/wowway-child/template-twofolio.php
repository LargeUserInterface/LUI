...
/* PLEASE READ!! There are three places where you want to change "twofolio" into your custom post type name.
	...
	1. At line 44: 'post_type' => 'twofolio'
	2. In the three places where it appears as 'twofolio_category'
	...
	<section id="portfolio" class="folioGrid <?php echo $thumbs_ratio; ?> get-ratio disable-resize-parent clearfix" data-url="<?php echo substr( get_permalink(), strpos( get_permalink(), '/', 9) ); ?>" data-gal="no" data-hover="<?php echo $thumbs_style; ?>">
		...
		<?php 
			...
			$args = array( 
				...
				'twofolio_category' => $portfolio_filter,
				'post_type' => 'twofolio',
				'paged' => $paged 
			);
		...
		<a id="post-<?php the_ID(); ?>" class="folioItem <?php krown_categories( $post->ID, 'twofolio_category', ' ', 'slug' ); ?> <?php echo $thumbs_style; ?> isotope-hidden" data-custom-filter="0" data-slug="<?php echo $post->post_name; ?>" data-title="<?php echo get_the_title() . ' | ' . get_bloginfo( 'name') ; ?>" <?php echo $item_href; ?>>
			...
			<div class="folioTextHolder">
				<div class="folioText">
					<h3><?php the_title(); ?></h3>
					<p><?php krown_categories( $post->ID, 'twofolio_category', ', ', 'name' ); ?></p>
				</div>
			</div>
